...
document.getElementById('btn-generate').addEventListener('click', async () => {
  ...
  try {
    ...
    const titlePart = eserAdi.trim().split(/\s+/).slice(0, 5).join('_').replace(/[\\/:*?"<>|;,]/g, '');
    const indeksFirst = (atiflar[0]?.indeks || '').trim().split(/\s+/)[0].replace(/[\\/:*?"<>|;,()]/g, '');
    link.download = (titlePart + (indeksFirst ? '_' + indeksFirst : '')) + '.pdf';
